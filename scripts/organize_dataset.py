...
Total polygons across all 46 countries: **{total_polygons:,}**.
...
A single parquet with **every** polygon from **every** country
concatenated. Schema is identical to the per-country parquets.

- `all_europe.parquet` — **{n_rows:,} polygons** across
  **{n_countries} countries** ({size_human}).
- Built from the per-country parquet files in [`../per_country/`](../per_country/).
- The `country` column tells you which country each row came from.
...
# ~9 GB uncompressed geometry; prefer pyarrow's row-group streaming
# for filtering:
filtered = table.filter(table["country"] == "france").to_pandas()
```

## When to use this vs per-country

| Use `combined/all_europe.parquet` | Use `per_country/<c>/<c>.parquet` |
|-----------------------------------|-----------------------------------|
| Cross-country analysis            | Single-country study              |
| Don't know the country in advance | Want to stream one country        |
| OK with ~9 GB download            | Want a smaller download           |
...
A **{n_rows:,}-polygon** spatial sample of the full European dataset,
designed to be **highly representative** of the geographic distribution
without being too dense to render in a browser. Use it as a quick
look at "what does this dataset look like?" without downloading the
9 GB combined parquet.

- `sample_map.jsonl` — JSONL with one polygon per line. Same schema as
  the per-country parquets, minus the geometry column (centroid-only).
...
Drop this into `scripts/visualize.py` (the existing map renderer) to
get an interactive folium map, or render it via the static preview
in [`../preview/`](../preview/).
...
A static thumbnail of the geographic distribution of the full dataset,
generated from [`../sample/sample_map.jsonl`](../sample/sample_map.jsonl).

- `map_preview.png` — 1600x1100 PNG, ~1 MB. The same image embedded
  in the root README's "Geographic distribution" section.

Generated by `scripts/render_map_screenshot.py`, which renders the
sample as a folium map and screenshots it via headless Chromium.
