...
Single source of truth for the dataset schema. Used by:
- ``build_dataset.py`` to write per-country parquets
- ``organize_dataset.py`` to render the schema table in READMEs
- ``make_split.py`` to add the ``split`` column

The geometry column is conditional: ``wkt`` (default, text),
``wkb`` (binary, ~50% smaller), or ``none`` (drop entirely). The
``split`` column is optional — included only after ``make_split``
has been run.
"""
