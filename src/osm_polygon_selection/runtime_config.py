"""Runtime configuration: filesystem roots for the build pipeline.
...
``default_data_root()`` and ``RuntimeConfig.from_env()`` consult env
vars on call.
"""
...
__all__ = ["RuntimeConfig", "default_data_root"]

_DEFAULT_HDD_ROOT = Path("/Volumes/Seagate M3/osm-polygon-selection")
