"""Public runner for the streaming JSONL -> parquet pipeline.

The :func:`write_jsonl_to_parquet` function lives here, not at
the root level. The root ``streaming_writer.py`` module is a
backwards-compat facade re-exporting it.
"""
...
# Re-export the Python fallback writer so legacy imports of
# ``streaming_writer._write_jsonl_to_parquet_python_json`` resolve
# through the sys.modules alias to this canonical module.
