...
# Pre-compiled pyarrow dtype to avoid re-allocating on every call.
_MATCHED_TAG_TYPE = None
...
def vectorized_compute_matched_tags(
    ...
    Example::
        ...
        tags = pa.array([["name=L", "natural=water"], ["place=x"]])
        out = vectorized_compute_matched_tags(tags)
        # out: ["natural=water", ""]
    """
    import numpy as np
    import pyarrow as pa
    ...
    for chunk in chunks:
        ...
        # np.minimum.reduceat requires non-empty slices; rows where
        # offsets[i+1] == offsets[i] (empty lists) need a fallback.
        n_rows = len(chunk)
        row_first = np.full(n_rows, n_flat + 1, dtype=np.int64)
        # Indices with non-empty slices
        ...
        # row_first is per-row, len = n_rows. If row_first > n_flat => no match.
